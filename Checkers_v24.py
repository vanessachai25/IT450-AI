...
class Checkers:
    ...
    def CompTurn(s):
        ...
        for move in s.moves:
            if not ((s.tiles[move[0]][move[1]].isKing) and (s.tiles[move[2]][move[3]].isKing) and not s.isMoveSafe(move)):
                s.badMoves.append(move)
        s.removeBadMoves()
        ...
        for move in s.moves:
            if not ((s.tiles[move[0]][move[1]].isPawn) and (s.tiles[move[2]][move[3]].isPawn) and not s.isMoveSafe(move)):
                s.badMoves.append(move)
        s.removeBadMoves()
        ...
        #To prevent leaving the back row 
        for move in s.moves:
